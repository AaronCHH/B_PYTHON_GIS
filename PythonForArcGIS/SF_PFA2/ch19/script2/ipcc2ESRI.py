...
try:
    ...
    inputFile = 'C:/gispy/data/ch19/precipitation6190.dat'
    inputDir = os.path.dirname(inputFile) + '/'
...
try:
    ...
    ###
        ...
        theDict = readHeader(inf)
        key = wordInKey(theDict, 'cols')
        numcols = int(theDict[key])
        key = wordInKey(theDict, 'rows')
        numrows = int(theDict[key])
        ...
        ###
            writeHeader(outf, theDict, cellsize, flag)
